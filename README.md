...
[SRCGI](https://github.com/Chouette2100/SRCGI)（[『SHOWROOM イベント 獲得ポイント一覧』(3) 獲得ポイントの推移の表示とデータ取得対象イベント・配信者の設定](https://zenn.dev/chouette2100/books/d8c28f8ff426b7/viewer/56ec9b)）でイベントを登録したときイベントに参加しているルームの情報が保存されます。
...
（今調べてみたら私の利用しているKAGOYA VPSでもUbuntu 22.04が選択できるようになっていました。そっちだとOSが開発環境と同じなので以下書いたようなCGO_ENABLEDがどうのこうのというトラブルには悩まされないかも）

まずGithubで入手[SRCGI](https://github.com/Chouette2100/SRCGI)でイベントを登録したときイベントに参加しているルームの情報が保存されます。
...
データベースのアカウント、パスワード等はServerConfig.ymlの方に直接書いてもいいのですが、ここでは環境変数を介して与える方法を使っています。これは単にServerConfig.ymlが公開されるパッケージに含まれているからというのが理由です。
